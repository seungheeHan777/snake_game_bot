...
GA 방식은 학습 중 최고점 `score=400`을 만든 사례가 있습니다. 즉, 단판 기준으로는 게임 성공이 가능했습니다.

하지만 같은 모델을 여러 번 반복 평가했을 때 안정적으로 `score=400`을 재현하지 못했습니다. 따라서 `ga_bot/`은 최종 안정 봇이 아니라 다음 역할로 유지합니다.

- 유전 알고리즘 실험 기록
- 기존 학습 방식의 baseline
- feature, fitness, mutation 실험 비교
- `stable_bot/`과 비교하기 위한 참고 구현

현재 안정적으로 완주하는 모델은 루트의 `stable_bot/`과 `snake_stable_bot.py`입니다.
