...
수동 플레이 결과 저장은 1차 구현이 완료되었습니다.

- 실행 파일: `snake_game.py`
- 저장 대상: pygame 창에서 직접 플레이한 결과
- 저장 방식: `players`에서 이름 기준 get-or-create 후 `game_runs`에 저장
- 구분 값: `actor_type=player`, `run_type=screen`
- 결과 화면: 이름 입력, `Save`, `Retry`, `Saved` 표시

다음 단계는 실제 화면 저장 동작을 수동 확인한 뒤, 시작 화면과 랭킹 화면을 설계하고 구현하는 것입니다.
